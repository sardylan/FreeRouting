...
import org.thehellnet.tools.freerouting.planar.FortyfiveDegreeBoundingDirections;
...
public class SearchTreeManager
{
    ...
    /**
     * Returns the default tree used in org.thehellnet.tools.freerouting.interactive routing.
     */
    ...
    /**
     * Clears all compensated trees used in the org.thehellnet.tools.freerouting.autoroute algorithm apart from the default tree.
     */
    ...
    private void remove_all_board_items()
    {
        if (this.board == null)
        {
            System.out.println("SearchtreeManager.remove_all_board_items: org.thehellnet.tools.freerouting.board is null");
            return;
        }
        Iterator<UndoableObjects.UndoableObjectNode> it = this.board.item_list.start_read_object();
        for(;;)
    ...
    private void insert_all_board_items()
    {
        if (this.board == null)
        {
            System.out.println("SearchtreeManager.insert_all_board_items: org.thehellnet.tools.freerouting.board is null");
            return;
        }
        Iterator<UndoableObjects.UndoableObjectNode> it = this.board.item_list.start_read_object();
        for(;;)
