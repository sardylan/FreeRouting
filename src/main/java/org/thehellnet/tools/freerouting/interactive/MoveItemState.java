...
public class MoveItemState extends InteractiveState
{
    ...
    {
        java.util.ResourceBundle resources = java.util.ResourceBundle.getBundle("interactive/InteractiveState", p_board_handling.get_locale());
        ...
        org.thehellnet.tools.freerouting.board.BasicBoard routing_board = p_board_handling.get_routing_board();
        ...
        for (Item curr_item : item_list)
        {
            ...
            else if (curr_item.is_connected())
            {
                ...
                Collection<Item> contacts = curr_item.get_connected_set(-1, true);
                {
                    ...
                    {
                        if (curr_contact instanceof org.thehellnet.tools.freerouting.board.ConductionArea)
                        {
            ...
            Component p_first_component, InteractiveState p_parent_state, BoardHandling p_board_handling, Logfile p_logfile)
    {
        ...
        org.thehellnet.tools.freerouting.board.BasicBoard routing_board = hdlg.get_routing_board();
    ...
    private void move(FloatPoint p_new_position)
    {
        ...
        if (!current_position.equals(previous_position))
        {
            ...
            org.thehellnet.tools.freerouting.board.Components components = hdlg.get_routing_board().components;
            for (Component curr_component : this.component_list)
            {
