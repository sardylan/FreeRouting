...
public class RouteState extends InteractiveState
{
    ...
    public static RouteState get_instance(FloatPoint p_location, InteractiveState p_parent_state, BoardHandling p_board_handling, Logfile p_logfile)
    {
        ...
        if (picked_item instanceof org.thehellnet.tools.freerouting.board.Pin && net_count > 1)
        {
            // tie pin, remove nets, which are already conneccted to this pin on the current layer.
            route_net_no_arr = get_route_net_numbers_at_tie_pin((org.thehellnet.tools.freerouting.board.Pin) picked_item, p_board_handling.settings.layer);
        }
        else
        {
        ...
        org.thehellnet.tools.freerouting.board.RoutingBoard routing_board = p_board_handling.get_routing_board();
        ...
        org.thehellnet.tools.freerouting.rules.Net curr_net = routing_board.rules.nets.get(route_net_no_arr[0]);
        if (curr_net == null)
        {
            return null;
        }
    ...
    /**
     * Checks starting an org.thehellnet.tools.freerouting.interactive route at p_location.
    ...
    {
        org.thehellnet.tools.freerouting.board.RoutingBoard routing_board = p_hdlg.get_routing_board();
    ...
    public InteractiveState key_typed(char p_key_char)
    {
        ...
        if (Character.isDigit(p_key_char))
        {
            ...
            org.thehellnet.tools.freerouting.board.LayerStructure layer_structure = hdlg.get_routing_board().layer_structure;
            ...
            d = Math.max(d - 1, 0);
            org.thehellnet.tools.freerouting.board.Layer new_layer = layer_structure.get_signal_layer(d);
     ...
     */
    static int[] get_route_net_numbers_at_tie_pin(org.thehellnet.tools.freerouting.board.Pin p_pin, int p_layer)
    {
    ...
    public void display_default_message()
    {
        if (route != null)
        {
            org.thehellnet.tools.freerouting.rules.Net curr_net = hdlg.get_routing_board().rules.nets.get(route.net_no_arr[0]);
