 ...
 */
package org.thehellnet.tools.freerouting.autoroute;

import org.thehellnet.tools.freerouting.planar.ConvexShape;
import org.thehellnet.tools.freerouting.planar.FloatLine;
...
import org.thehellnet.tools.freerouting.planar.Line;
...
import org.thehellnet.tools.freerouting.board.Connectable;
import org.thehellnet.tools.freerouting.board.ForcedViaAlgo;
import org.thehellnet.tools.freerouting.board.Item;
import org.thehellnet.tools.freerouting.board.PolylineTrace;
import org.thehellnet.tools.freerouting.board.ShapeSearchTree;
import org.thehellnet.tools.freerouting.board.AngleRestriction;
import org.thehellnet.tools.freerouting.board.SearchTreeObject;
...
public class MazeSearchAlgo
{
    ...
    private boolean expand_to_room_doors(MazeListElement p_list_element)
    {
        ...
        if (this.ctrl.with_neckdown && p_list_element.door instanceof TargetItemExpansionDoor)
        {
            ...
            if (start_item instanceof org.thehellnet.tools.freerouting.board.Pin)
            {
                double neckdown_half_width = ((org.thehellnet.tools.freerouting.board.Pin) start_item).get_trace_neckdown_halfwidth(layer_no);
        ...
        else if (p_list_element.next_room instanceof ObstacleExpansionRoom)
        {
            ...
            if (!p_list_element.already_checked)
            {
                ...
                if (ripup_costs != ALREADY_RIPPED_COSTS && next_room_is_thick)
                {
                    ...
                    if (!curr_door_is_small && this.ctrl.max_shove_trace_recursion_depth > 0 && obstacle_item instanceof org.thehellnet.tools.freerouting.board.PolylineTrace)
                    {
        ...
        if (ctrl.vias_allowed && !(p_list_element.door instanceof ExpansionDrill))
        {
            ...
            else if (p_list_element.next_room instanceof ObstacleExpansionRoom)
            {
                ...
                if (curr_obstacle_item instanceof org.thehellnet.tools.freerouting.board.Via)
                {
                    org.thehellnet.tools.freerouting.board.Via curr_via = (org.thehellnet.tools.freerouting.board.Via) curr_obstacle_item;
    ...
    private boolean door_is_small(ExpansionDoor p_door, double p_trace_width)
    {
        if (p_door.dimension == 1 || p_door.first_room instanceof CompleteFreeSpaceExpansionRoom && p_door.second_room instanceof CompleteFreeSpaceExpansionRoom)
        {
            ...
            {
                if (this.autoroute_engine.board.get_test_level().ordinal() >= org.thehellnet.tools.freerouting.board.TestLevel.ALL_DEBUGGING_OUTPUT.ordinal())
                {
    ...
    private void expand_to_drill(ExpansionDrill p_drill, MazeListElement p_from_element, int p_add_costs)
    {
        ...
        if (p_from_element.door instanceof DrillPage && p_from_element.backtrack_door instanceof TargetItemExpansionDoor)
        {
            ...
            if (from_item instanceof org.thehellnet.tools.freerouting.board.Pin)
            {
                FloatPoint nearest_exit_corner = ((org.thehellnet.tools.freerouting.board.Pin) from_item).nearest_trace_exit_corner(p_drill.location.to_float(), trace_half_width, layer);
    ...
    private void expand_to_other_layers(MazeListElement p_list_element)
    {
        ...
        if (curr_drill.room_arr[p_list_element.section_no_of_door] instanceof ObstacleExpansionRoom)
        {
            ...
            org.thehellnet.tools.freerouting.library.Padstack curr_obstacle_padstack = ((org.thehellnet.tools.freerouting.board.Via) curr_obstacle_item).get_padstack();
        ...
        else
        {
            ...
            for (;;)
            {
                ...
                org.thehellnet.tools.freerouting.board.ForcedPadAlgo.CheckDrillResult drill_result =
                ...
                if (drill_result == org.thehellnet.tools.freerouting.board.ForcedPadAlgo.CheckDrillResult.NOT_DRILLABLE)
                {
                    via_lower_bound = curr_layer + 1;
                    break;
                }
                else if (drill_result == org.thehellnet.tools.freerouting.board.ForcedPadAlgo.CheckDrillResult.DRILLABLE_WITH_ATTACH_SMD)
                {
                    if (curr_layer == 0)
                    {
    ...
    private boolean init(Set<Item> p_start_items, Set<Item> p_destination_items)
    {
        ...
        {
            // add the completed start rooms carried over from the last org.thehellnet.tools.freerouting.autoroute to the start rooms.
    ...
    private static void reduce_trace_shapes_at_tie_pins(Collection<Item> p_item_list, int p_own_net_no, ShapeSearchTree p_autoroute_tree)
    {
        ...
        {
            if ((curr_item instanceof org.thehellnet.tools.freerouting.board.Pin) && curr_item.net_count() > 1)
            {
                Collection<Item> pin_contacts = curr_item.get_normal_contacts();
                org.thehellnet.tools.freerouting.board.Pin curr_tie_pin = (org.thehellnet.tools.freerouting.board.Pin) curr_item;
                for (Item curr_contact : pin_contacts)
                {
                    if (!(curr_contact instanceof org.thehellnet.tools.freerouting.board.PolylineTrace) || curr_contact.contains_net(p_own_net_no))
                    {
                        continue;
                    }
                    p_autoroute_tree.reduce_trace_shape_at_tie_pin(curr_tie_pin, (org.thehellnet.tools.freerouting.board.PolylineTrace) curr_contact);
                }
            }
        }
    }
    ...
    private boolean room_shape_is_thick(ObstacleExpansionRoom p_obstacle_room)
    {
        ...
        if (obstacle_item instanceof org.thehellnet.tools.freerouting.board.Trace)
        {
            obstacle_half_width = ((org.thehellnet.tools.freerouting.board.Trace) obstacle_item).get_half_width()
                    + this.search_tree.clearance_compensation_value(obstacle_item.clearance_class_no(), layer);

        }
        else if (obstacle_item instanceof org.thehellnet.tools.freerouting.board.Via)
        {
            TileShape via_shape = ((org.thehellnet.tools.freerouting.board.Via) obstacle_item).get_tree_shape_on_layer(this.search_tree, layer);
    ...
    private int check_ripup(MazeListElement p_list_element, Item p_obstacle_item, boolean p_door_is_small)
    {
        ...
        if (p_obstacle_item instanceof org.thehellnet.tools.freerouting.board.Trace)
        {
            org.thehellnet.tools.freerouting.board.Trace obstacle_trace = (org.thehellnet.tools.freerouting.board.Trace) p_obstacle_item;
        ...
        else if (p_obstacle_item instanceof org.thehellnet.tools.freerouting.board.Via)
        {
            ...
            {
                if (!(curr_contact instanceof org.thehellnet.tools.freerouting.board.Trace) || curr_contact.is_user_fixed())
                {
                    return -1;
                }
                ++contact_count;
                org.thehellnet.tools.freerouting.board.Trace obstacle_trace = (org.thehellnet.tools.freerouting.board.Trace) curr_contact;
     ...
     */
    private static double calc_fanout_via_ripup_cost_factor(org.thehellnet.tools.freerouting.board.Trace p_trace)
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            if (curr_trace_contact instanceof org.thehellnet.tools.freerouting.board.Pin && curr_trace_contact.first_layer() == curr_trace_contact.last_layer())
            {
                protect_fanout_via = true;
            }
            else if (curr_trace_contact instanceof PolylineTrace && curr_trace_contact.get_fixed_state() == org.thehellnet.tools.freerouting.board.FixedState.SHOVE_FIXED)
            {
    ...
    private double check_neck_down_at_dest_pin(CompleteExpansionRoom p_room)
    {
        ...
        {
            if (curr_target_door.item instanceof org.thehellnet.tools.freerouting.board.Pin)
            {
                return ((org.thehellnet.tools.freerouting.board.Pin) curr_target_door.item).get_trace_neckdown_halfwidth(p_room.get_layer());
            }
        }
        return 0;
    }
    ...
    /**
     * The org.thehellnet.tools.freerouting.autoroute engine of this expansion algorithm.
     */
