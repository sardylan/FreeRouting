...
public class ExpansionDrill implements ExpandableObject
{
    ...
    public boolean calculate_expansion_rooms(AutorouteEngine p_autoroute_engine)
    {
        ...
        Collection<org.thehellnet.tools.freerouting.board.SearchTreeObject> overlaps =
        ...
        for (int i = this.first_layer; i <= this.last_layer; ++i)
        {
            ...
            Iterator<org.thehellnet.tools.freerouting.board.SearchTreeObject> it = overlaps.iterator();
            while (it.hasNext())
            {
                org.thehellnet.tools.freerouting.board.SearchTreeObject curr_ob = it.next();
