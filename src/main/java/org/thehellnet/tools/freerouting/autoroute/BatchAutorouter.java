...
import org.thehellnet.tools.freerouting.interactive.BoardHandling;
import org.thehellnet.tools.freerouting.interactive.InteractiveActionThread;

/**
 * Handles the sequencing of the batch org.thehellnet.tools.freerouting.autoroute passes.
 * 
 * @author  Alfons Wirtz
 */
public class BatchAutorouter
{
    ...
    /**
     *  Autoroutes ripup passes until the org.thehellnet.tools.freerouting.board is completed or the autorouter is stopped by the user,
     *  or if p_max_pass_count is exceeded. Is currently used in the optimize via batch pass.
     *  Returns the number of oasses to complete the org.thehellnet.tools.freerouting.board or p_max_pass_count + 1,
     *  if the org.thehellnet.tools.freerouting.board is not completed.
     */
    ...
    /**
     *  Autoroutes ripup passes until the org.thehellnet.tools.freerouting.board is completed or the autorouter is stopped by the user.
     *  Returns true if the org.thehellnet.tools.freerouting.board is completed.
     */
    public boolean autoroute_passes()
    {
        java.util.ResourceBundle resources =
                java.util.ResourceBundle.getBundle("interactive/InteractiveState", hdlg.get_locale());
        boolean still_unrouted_items = true;
        ...
        {
            // clean up the route if the org.thehellnet.tools.freerouting.board is completed and if fanout is used.
    ...
    /**
     * Autoroutes one ripup pass of all items of the org.thehellnet.tools.freerouting.board.
     * Returns false, if the org.thehellnet.tools.freerouting.board is already completely routed.
     */
    private boolean autoroute_pass(int p_pass_no, boolean p_with_screen_message)
    {
        try
        {
            ...
            if (routing_board.get_test_level() != org.thehellnet.tools.freerouting.board.TestLevel.ALL_DEBUGGING_OUTPUT)
            {
    ...
    private boolean autoroute_item(Item p_item, int p_route_net_no, SortedSet<Item> p_ripped_item_list, int p_ripup_pass_no)
    {
        try
        {
            ...
            org.thehellnet.tools.freerouting.rules.Net route_net = routing_board.rules.nets.get(p_route_net_no);
            if (route_net != null)
            {
            ...
            // org.thehellnet.tools.freerouting.tests.Validate.check("Autoroute  ", hdlg.get_routing_board());
