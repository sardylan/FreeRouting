...
package org.thehellnet.tools.freerouting.tests;
...
public class Validate
{
    ...
    {
        if (p_board.get_test_level() == org.thehellnet.tools.freerouting.board.TestLevel.RELEASE_VERSION)
        {
            return true;
        }
        boolean result = true;
        ...
        for (int layer = 0; layer < layer_count; ++layer)
        {
            if (first_time)
            {
                System.out.println(" validate org.thehellnet.tools.freerouting.board is on ");
                first_time = false;
            }
    ...
    static public boolean has_cycles(String p_s, BasicBoard p_board)
    {
        ...
        while (it.hasNext())
        {
            ...
            if(((org.thehellnet.tools.freerouting.board.Trace)curr_item).is_cycle())
            {
                System.out.print(p_s);
    ...
    static public boolean trace_count_exceeded(String p_s, BasicBoard p_board, int p_net_no, int p_max_count)
    {
        ...
        {
            Item curr_ob = it.next();
            if(curr_ob instanceof org.thehellnet.tools.freerouting.board.Trace)
            {
                if (((org.thehellnet.tools.freerouting.board.Trace)curr_ob).contains_net(p_net_no))
                {
                    ++found_traces;
                }
            }
    ...
    /**
     * checks, if there are unconnectedtraces ore vias on the org.thehellnet.tools.freerouting.board
     */
