...
import org.thehellnet.tools.freerouting.board.BasicBoard;
...
public class DsnFile
{
    ...
    /**
     * Creates a routing org.thehellnet.tools.freerouting.board from a Specctra dns file.
     ...
     */
    public static ReadResult read(java.io.InputStream p_input_stream, org.thehellnet.tools.freerouting.interactive.BoardHandling p_board_handling,
                                  org.thehellnet.tools.freerouting.board.BoardObservers p_observers, org.thehellnet.tools.freerouting.datastructures.IdNoGenerator p_item_id_no_generator, TestLevel p_test_level)
    {
        ...
        if (read_ok)
        {
            ...
            {
                // look for power planes with incorrect layer type and adjust org.thehellnet.tools.freerouting.autoroute parameters
        ...
        //org.thehellnet.tools.freerouting.tests.Validate.check("after reading dsn", read_scope_par.board_handling.get_routing_board());
        return result;
    }
     ...
     */
    private static boolean adjust_plane_autoroute_settings(org.thehellnet.tools.freerouting.interactive.BoardHandling p_board_handling)
    {
        BasicBoard routing_board = p_board_handling.get_routing_board();
        org.thehellnet.tools.freerouting.board.LayerStructure board_layer_structure = routing_board.layer_structure;
        if (board_layer_structure.arr.length <= 2)
        {
            return false;
        }
        for (org.thehellnet.tools.freerouting.board.Layer curr_layer : board_layer_structure.arr)
        {
        ...
        java.util.Collection<org.thehellnet.tools.freerouting.board.ConductionArea> conduction_area_list = new java.util.LinkedList<org.thehellnet.tools.freerouting.board.ConductionArea>();
        java.util.Collection<org.thehellnet.tools.freerouting.board.Item> item_list = routing_board.get_items();
        for (org.thehellnet.tools.freerouting.board.Item curr_item : item_list)
        {
            if (curr_item instanceof org.thehellnet.tools.freerouting.board.Trace)
            {
                int curr_layer = ((org.thehellnet.tools.freerouting.board.Trace) curr_item).get_layer();
                layer_contains_wires_arr[curr_layer] = true;
            }
            else if (curr_item instanceof org.thehellnet.tools.freerouting.board.ConductionArea)
            {
                conduction_area_list.add((org.thehellnet.tools.freerouting.board.ConductionArea) curr_item);
            }
        }
        boolean nothing_changed = true;

        org.thehellnet.tools.freerouting.board.BoardOutline board_outline = routing_board.get_outline();
        double board_area = 0;
        for (int i = 0; i < board_outline.shape_count(); ++i)
        {
            org.thehellnet.tools.freerouting.planar.TileShape[] curr_piece_arr = board_outline.get_shape(i).split_to_convex();
            if (curr_piece_arr != null)
            {
                for (org.thehellnet.tools.freerouting.planar.TileShape curr_piece : curr_piece_arr)
                {
                    board_area += curr_piece.area();
                }
            }
        }
        for (org.thehellnet.tools.freerouting.board.ConductionArea curr_conduction_area : conduction_area_list)
        {
            ...
            org.thehellnet.tools.freerouting.board.Layer curr_layer = routing_board.layer_structure.arr[layer_no];
            if (!curr_layer.is_signal || layer_no == 0 || layer_no == board_layer_structure.arr.length - 1)
            {
                continue;
            }
            org.thehellnet.tools.freerouting.planar.TileShape[] convex_pieces = curr_conduction_area.get_area().split_to_convex();
            double curr_area = 0;
            for (org.thehellnet.tools.freerouting.planar.TileShape curr_piece : convex_pieces)
            {
                curr_area += curr_piece.area();
            }
            if (curr_area < 0.5 * board_area)
            {
                // skip conduction areas not covering most of the org.thehellnet.tools.freerouting.board
                continue;
            }

            for (int i = 0; i < curr_conduction_area.net_count(); ++i)
            {
                org.thehellnet.tools.freerouting.rules.Net curr_net = routing_board.rules.nets.get(curr_conduction_area.get_net_no(i));
                curr_net.set_contains_plane(true);
                nothing_changed = false;
            }

            changed_layer_arr[layer_no] = true;
            if (curr_conduction_area.get_fixed_state().ordinal() < org.thehellnet.tools.freerouting.board.FixedState.USER_FIXED.ordinal())
            {
                curr_conduction_area.set_fixed_state(org.thehellnet.tools.freerouting.board.FixedState.USER_FIXED);
            }
        }
        if (nothing_changed)
        {
        ...
        // Adjust the layer prefered directions in the org.thehellnet.tools.freerouting.autoroute settings.
        // and deactivate the changed layers.
        org.thehellnet.tools.freerouting.interactive.AutorouteSettings autoroute_settings = p_board_handling.settings.autoroute_settings;
     ...
     */
    public static boolean write(org.thehellnet.tools.freerouting.interactive.BoardHandling p_board_handling, java.io.OutputStream p_file, String p_design_name, boolean p_compat_mode)
    {
        //org.thehellnet.tools.freerouting.tests.Validate.check("before writing dsn", p_board);
    ...
    private static void write_pcb_scope(org.thehellnet.tools.freerouting.interactive.BoardHandling p_board_handling, IndentFileWriter p_file, String p_design_name, boolean p_compat_mode)
