...
import org.thehellnet.tools.freerouting.board.ConductionArea;
...
public class SessionFile
{
    ...
    public static void write_components(BasicBoard p_board, IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform,
            IndentFileWriter p_file, org.thehellnet.tools.freerouting.library.Package p_package) throws java.io.IOException
    {
        Collection<org.thehellnet.tools.freerouting.board.Item> board_items =  p_board.get_items();
        boolean component_found = false;
        for (int i = 1; i <= p_board.components.count(); ++i)
        {
            org.thehellnet.tools.freerouting.board.Component curr_component =  p_board.components.get(i);
            if (curr_component.get_package() == p_package)
            {
                ...
                Iterator<org.thehellnet.tools.freerouting.board.Item> it = board_items.iterator();
                while (it.hasNext())
                {
                    org.thehellnet.tools.freerouting.board.Item curr_item = it.next();
    ...
    public static void write_component(BasicBoard p_board, IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform,
            IndentFileWriter p_file, org.thehellnet.tools.freerouting.board.Component p_component) throws java.io.IOException
    {
        p_file.new_line();
        p_file.write("(place ");
            ...
            IndentFileWriter p_file) throws java.io.IOException
    {
        p_file.start_scope();
        p_file.write("was_is");
        Collection<org.thehellnet.tools.freerouting.board.Pin> board_pins = p_board.get_pins();
        for (org.thehellnet.tools.freerouting.board.Pin curr_pin : board_pins)
        {
            org.thehellnet.tools.freerouting.board.Pin swapped_with = curr_pin.get_changed_to();
            if (curr_pin.get_changed_to() != curr_pin)
            {
                p_file.new_line();
                p_file.write("(pins ");
                org.thehellnet.tools.freerouting.board.Component curr_cmp = p_board.components.get(curr_pin.get_component_no());
                if (curr_cmp != null)
                {
                    p_identifier_type.write(curr_cmp.name, p_file);
                    p_file.write("-");
                    org.thehellnet.tools.freerouting.library.Package.Pin package_pin = curr_cmp.get_package().get_pin(curr_pin.get_index_in_package());
                ...
                p_file.write(" ");
                org.thehellnet.tools.freerouting.board.Component swap_cmp = p_board.components.get(swapped_with.get_component_no());
                if (swap_cmp != null)
                {
                    p_identifier_type.write(swap_cmp.name, p_file);
                    p_file.write("-");
                    org.thehellnet.tools.freerouting.library.Package.Pin package_pin = swap_cmp.get_package().get_pin(swapped_with.get_index_in_package());
                    p_identifier_type.write(package_pin.name, p_file);
                }
                else
                {
    ...
    private static void write_padstack(org.thehellnet.tools.freerouting.library.Padstack p_padstack, BasicBoard p_board, IdentifierType p_identifier_type,
            ...
            throws java.io.IOException
    {
        ...
        for (int i = first_layer_no; i <= last_layer_no; ++i)
        {
            ...
            org.thehellnet.tools.freerouting.board.Layer board_layer = p_board.layer_structure.arr[i];
    ...
    private static void write_net(int p_net_no, BasicBoard p_board, IdentifierType p_identifier_type, CoordinateTransform p_coordinate_transform,
            IndentFileWriter p_file) throws java.io.IOException
    {
        Collection<org.thehellnet.tools.freerouting.board.Item> net_items = p_board.get_connectable_items(p_net_no);
        boolean header_written = false;
        Iterator<org.thehellnet.tools.freerouting.board.Item> it = net_items.iterator();
        while (it.hasNext())
        {
            org.thehellnet.tools.freerouting.board.Item curr_item = it.next();
            if (curr_item.get_fixed_state() == org.thehellnet.tools.freerouting.board.FixedState.SYSTEM_FIXED)
            {
                continue;
            }
            ...
            if (!header_written && (is_wire || is_via || is_conduction_area))
            {
                p_file.start_scope();
                p_file.write("net ");
                org.thehellnet.tools.freerouting.rules.Net curr_net = p_board.rules.nets.get(p_net_no);
                if (curr_net == null)
                {
            ...
            CoordinateTransform p_coordinate_transform, IndentFileWriter p_file) throws java.io.IOException
    {
        ...
        org.thehellnet.tools.freerouting.board.Layer board_layer = p_board.layer_structure.arr[layer_no];
    ...
    static private void write_fixed_state(IndentFileWriter p_file, org.thehellnet.tools.freerouting.board.FixedState p_fixed_state) throws java.io.IOException
    {
        if (p_fixed_state.ordinal() <= org.thehellnet.tools.freerouting.board.FixedState.SHOVE_FIXED.ordinal())
        {
            return;
        }
        p_file.new_line();
        p_file.write("(type ");
        if (p_fixed_state == org.thehellnet.tools.freerouting.board.FixedState.SYSTEM_FIXED)
        {
            p_file.write("fix)");
        }
        else
