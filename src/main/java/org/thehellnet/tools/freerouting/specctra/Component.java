...
/**
 * Handels the placement bata of a org.thehellnet.tools.freerouting.library component.
 *
 * @author  alfons
 */
public class Component extends ScopeKeyword
{
    ...
    public static void write_scope(WriteScopeParameter p_par, org.thehellnet.tools.freerouting.board.Component p_component)
    ...
    private static void write_pin_info(WriteScopeParameter p_par, org.thehellnet.tools.freerouting.board.Component p_component, int p_pin_no)
    throws java.io.IOException
    {
        if (!p_component.is_placed())
        {
        ...
        org.thehellnet.tools.freerouting.library.Package.Pin package_pin = p_component.get_package().get_pin(p_pin_no);
        if (package_pin == null)
        {
            System.out.println("Component.write_pin_info: package pin not found");
            return;
        }
        org.thehellnet.tools.freerouting.board.Pin component_pin = p_par.board.get_pin(p_component.no, p_pin_no);
    ...
    private static void write_keepout_infos(WriteScopeParameter p_par, org.thehellnet.tools.freerouting.board.Component p_component)
    throws java.io.IOException
    {
        if (!p_component.is_placed())
        {
        ...
        org.thehellnet.tools.freerouting.library.Package.Keepout[] curr_keepout_arr;
        String keepout_type;
        for (int j = 0; j < 3; ++j)
        {
            if (j == 0)
            ...
            {
                org.thehellnet.tools.freerouting.library.Package.Keepout curr_keepout = curr_keepout_arr[i];
                org.thehellnet.tools.freerouting.board.ObstacleArea curr_obstacle_area = get_keepout(p_par.board, p_component.no, curr_keepout.name);
    ...
    private static org.thehellnet.tools.freerouting.board.ObstacleArea get_keepout(org.thehellnet.tools.freerouting.board.BasicBoard p_board, int p_component_no, String p_name)
    {
        java.util.Iterator<org.thehellnet.tools.freerouting.datastructures.UndoableObjects.UndoableObjectNode> it = p_board.item_list.start_read_object();
        for(;;)
        {
            org.thehellnet.tools.freerouting.board.Item curr_item = (org.thehellnet.tools.freerouting.board.Item)p_board.item_list.read_object(it);
            if (curr_item == null)
            {
                break;
            }
            if (curr_item.get_component_no() == p_component_no && curr_item instanceof org.thehellnet.tools.freerouting.board.ObstacleArea)
            {
                org.thehellnet.tools.freerouting.board.ObstacleArea curr_area = (org.thehellnet.tools.freerouting.board.ObstacleArea) curr_item;
