...
/**
 * Window handling snapshots of the org.thehellnet.tools.freerouting.interactive situation.
 *
 * @author  Alfons Wirtz
 */
public class WindowSnapshot extends BoardSavableSubWindow
{
    ...
    public WindowSnapshot(BoardFrame p_board_frame)
    {
        ...
        this.resources = java.util.ResourceBundle.getBundle("gui/WindowSnapshot", p_board_frame.get_locale());
        ...
        if (index >= 0 && list_model.getSize() > index)
        {
            ...
            if (curr_snapshot.settings.get_snapshot_attributes().object_colors)
            {
                board_handling.graphics_context.item_color_table =
                        new org.thehellnet.tools.freerouting.boardgraphics.ItemColorTableModel(curr_snapshot.graphics_context.item_color_table);
                board_handling.graphics_context.other_color_table =
                        new org.thehellnet.tools.freerouting.boardgraphics.OtherColorTableModel(curr_snapshot.graphics_context.other_color_table);
