 ...
 * @author Alfons Wirtz
 */
public class WindowObjectInfo extends BoardTemporarySubWindow implements org.thehellnet.tools.freerouting.board.ObjectInfoPanel
{
    /**
     * Displays a new ObjectInfoWindow with information about the items in p_item_list.
     * p_coordinate_transform is for transforming org.thehellnet.tools.freerouting.board to user coordinates,
     * and p_location is the location of the window.
     */
    public static void display(Collection<org.thehellnet.tools.freerouting.board.Item> p_item_list,
                               BoardFrame p_board_frame, org.thehellnet.tools.freerouting.board.CoordinateTransform p_coordinate_transform, java.awt.Point p_location)
    {
        ...
        for (WindowObjectInfo.Printable curr_object : p_item_list)
        {
            ...
            if (curr_object instanceof org.thehellnet.tools.freerouting.board.Pin)
            {
                ++pin_count;
            }
            else if (curr_object instanceof org.thehellnet.tools.freerouting.board.Via)
            {
                ++via_count;
            }
            else if (curr_object instanceof org.thehellnet.tools.freerouting.board.Trace)
            {
                ++trace_count;
                cumulative_trace_length += ((org.thehellnet.tools.freerouting.board.Trace) curr_object).get_length();
            }
        }
    ...
    public static WindowObjectInfo display(String p_title, Collection<Printable> p_object_list,
            BoardFrame p_board_frame, org.thehellnet.tools.freerouting.board.CoordinateTransform p_coordinate_transform)
    {
    ...
    private WindowObjectInfo(BoardFrame p_board_frame, org.thehellnet.tools.freerouting.board.CoordinateTransform p_coordinate_transform)
    {
        super(p_board_frame);
        this.resources = 
                java.util.ResourceBundle.getBundle("gui/WindowObjectInfo", p_board_frame.get_locale());
     ...
     * Returns false, if that was not possible.
     */
    public boolean append(org.thehellnet.tools.freerouting.planar.FloatPoint p_point)
    {
        org.thehellnet.tools.freerouting.planar.FloatPoint transformed_point = this.coordinate_transform.board_to_user(p_point);
     ...
     * Returns false, if that was not possible.
     */
    public boolean append(org.thehellnet.tools.freerouting.planar.Shape p_shape, java.util.Locale p_locale)
    {
        org.thehellnet.tools.freerouting.board.PrintableShape transformed_shape = this.coordinate_transform.board_to_user(p_shape, p_locale);
     ...
     */
    public boolean append_items( String p_button_name, String p_window_title, java.util.Collection<org.thehellnet.tools.freerouting.board.Item> p_items)
    {
        java.util.Collection<WindowObjectInfo.Printable> object_list = new java.util.LinkedList<WindowObjectInfo.Printable>();
    ...
    private final org.thehellnet.tools.freerouting.board.CoordinateTransform coordinate_transform;
