...
/**
 *  Window handling parameters of the org.thehellnet.tools.freerouting.interactive routing.
 *
 * @author  Alfons Wirtz
 */
public class WindowRouteParameter extends BoardSavableSubWindow
{
    ...
    public WindowRouteParameter(BoardFrame p_board_frame)
    {
        ...
        java.util.ResourceBundle resources = 
                java.util.ResourceBundle.getBundle("gui/WindowRouteParameter", p_board_frame.get_locale());
        ...
        if (this.board_handling.get_routing_board().get_test_level() != org.thehellnet.tools.freerouting.board.TestLevel.RELEASE_VERSION)
        {
    ...
    public void refresh()
    {
        org.thehellnet.tools.freerouting.board.AngleRestriction snap_angle = this.board_handling.get_routing_board().rules.get_trace_angle_restriction();
        
        if (snap_angle == org.thehellnet.tools.freerouting.board.AngleRestriction.NINETY_DEGREE)
        {
            snap_angle_90_button.setSelected(true);
        }
        else if (snap_angle == org.thehellnet.tools.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE)
        {
    ...
    private class SnapAngle90Listener implements java.awt.event.ActionListener
    {
        ...
        {
            if (board_handling.get_routing_board().rules.get_trace_angle_restriction() == org.thehellnet.tools.freerouting.board.AngleRestriction.NINETY_DEGREE)
            {
                return;
            }
            Collection<org.thehellnet.tools.freerouting.board.Trace> trace_list = board_handling.get_routing_board().get_traces();
            boolean free_angle_traces_found = false;
            for (org.thehellnet.tools.freerouting.board.Trace curr_trace : trace_list)
            {
                if (curr_trace instanceof org.thehellnet.tools.freerouting.board.PolylineTrace)
                {
                    if (!((org.thehellnet.tools.freerouting.board.PolylineTrace)curr_trace).polyline().is_orthogonal())
                    {
            ...
            {
                java.util.ResourceBundle resources = 
                        java.util.ResourceBundle.getBundle("gui/WindowRouteParameter", current_locale);
            ...
            board_handling.set_current_snap_angle(org.thehellnet.tools.freerouting.board.AngleRestriction.NINETY_DEGREE);
        }
    }
    ...
    private class SnapAngle45Listener implements java.awt.event.ActionListener
    {
        public void actionPerformed(java.awt.event.ActionEvent p_evt)
        {
            if (board_handling.get_routing_board().rules.get_trace_angle_restriction() == org.thehellnet.tools.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE)
            {
                return;
            }
            Collection<org.thehellnet.tools.freerouting.board.Trace> trace_list = board_handling.get_routing_board().get_traces();
            ...
            for (org.thehellnet.tools.freerouting.board.Trace curr_trace : trace_list)
            {
                if (curr_trace instanceof org.thehellnet.tools.freerouting.board.PolylineTrace)
                {
                    if (!((org.thehellnet.tools.freerouting.board.PolylineTrace)curr_trace).polyline().is_multiple_of_45_degree())
                    {
                        free_angle_traces_found = true;
                        break;
                    }
            ...
            board_handling.set_current_snap_angle(org.thehellnet.tools.freerouting.board.AngleRestriction.FORTYFIVE_DEGREE);
        }
    }
    ...
    private class SnapAngleNoneListener implements java.awt.event.ActionListener
    {
        public void actionPerformed(java.awt.event.ActionEvent p_evt)
        {
            board_handling.set_current_snap_angle(org.thehellnet.tools.freerouting.board.AngleRestriction.NONE);
        }
    }
