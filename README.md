...
There are three modes for routing traces: 90 degree, 45 degree and free angle. The org.thehellnet.tools.freerouting.interactive router is production stable and unsurpassed in its free angle capabilities. An autorouter is currently under development and already stable in the conventional 45 degree mode.
...
After opening a design you can start the autorouter with the button in the toolbar on top of the org.thehellnet.tools.freerouting.board window.

The org.thehellnet.tools.freerouting.board editor has three different org.thehellnet.tools.freerouting.interactive states. You can switch between this states with the buttons Select, Route and Drag on the left of the toolbar.

In the beginning the org.thehellnet.tools.freerouting.board editor is in the select state. In this state you can select single org.thehellnet.tools.freerouting.board items by picking them with the left mouse button or select items in a rectangle by dragging the left mouse button. Only item types switched on in the select parameter sheet will be selected. After selecting some items the toolbar displays options for showing and manipulating these items. If you push the info button for example a window with text information about the selected items is displayed. After clicking a blue word in this text a new window with further information pops up. To return to the select state push the cancel button or click somewhere in the empty space of the org.thehellnet.tools.freerouting.board window.

By pushing the Route button you get into the state for org.thehellnet.tools.freerouting.interactive routing. In this state you can start a new trace by picking an item belonging to a net, for example a pin. Then you can follow the displayed airline with the mouse until you have reached the target item at the other end of the airline. The trace will be connected automatically to the target, if it is on the same layer. If you want to change to a different layer during org.thehellnet.tools.freerouting.interactive routing, select "change layer" and then the name of the new layer in the popup menu under the right mouse button. Then a via will be inserted, if that is possible, and a new trace starts on the new layer. You can also change the layer by pressing a number key.
...
For more information please use the online help in the org.thehellnet.tools.freerouting.board editor. From here you can download also a printable version of the online help.
...
6) To get rid of the undefined's download and unzip the attached org.thehellnet.tools.freerouting.library jh.jar. It is the system org.thehellnet.tools.freerouting.library of the Java Help system. 
...
For optional parameters of the Freerouting outfile check the usage of the variable p_args in the source file org.thehellnet.tools.freerouting.gui/MainApplication.java.
